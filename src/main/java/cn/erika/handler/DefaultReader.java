...
public class DefaultReader implements Reader {
    ...
    private byte[] cacheTmp2 = new byte[0];
    // 缓冲区写入偏移量
    ...
    public void process(byte[] data, int len) throws IOException {
        ...
        if (cache.length == 0) {
            if (cacheTmp.length == 0) {
                ...
                cacheTmp = new byte[cacheTmp2.length + len];
                System.arraycopy(cacheTmp2, 0, cacheTmp, 0, cacheTmp2.length);
                System.arraycopy(data, 0, cacheTmp, cacheTmp2.length, len);
                getHead(cacheTmp, cacheTmp.length);
            }
        } else {
            // 如果缓冲区写入偏移量加上本次传来的数据大于缓冲区长度
            // 说明这次传来的数据包含两轮数据的一部分
            if (cachePos + len > cache.length) {
                // 先把缓冲区装满再说
                // 装满之后 剩下没装的就是下一轮数据的请求头加数据部分
                int available = cache.length - cachePos;
                System.arraycopy(data, 0, cache, cachePos, available);
                cachePos += available;
                // 将缓冲区交给处理器处理
                ...
                if (len - available >= DataHead.LEN) {
                    ...
                    cacheTmp2 = new byte[len - available];
                    System.arraycopy(data, available, cacheTmp2, 0, len - available);
                    cachePos = 0;
                }
            } else {
                // 如果缓冲区能装下的话 就塞进缓冲区
                System.arraycopy(data, 0, cache, cachePos, len);
                cachePos += len;
            }
        }
        // 检查数据 如果写入偏移量刚好等于缓冲区容量 而且都不是0的话
        // 或者请求头中数据长度就是0 说明这一轮的数据都拿到了 开始处理并清空缓冲区,偏移量归零,数据头置空
        if (cachePos == cache.length && cachePos > 0 || head.getLen() == 0) {
            log.debug("数据完整 开始处理");
            handler.deal(socket, head, cache);
            cache = new byte[0];
            cachePos = 0;
    ...
    // 获取数据头 并将剩余的部分丢回缓冲区
    private void getHead(byte[] data, int len) throws IOException {
        try {
            // 把数据头提取出来
            log.debug("读取数据头");
            ...
            cachePos = len - DataHead.LEN;
        } catch (Exception e) {
            throw new IOException("不是预期的数据: " + new String(data, charset));
        }
    }
}
